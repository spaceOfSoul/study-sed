...
#에러 나는 이유    strong => 2차원 행렬 (time만 옮긴거니까 strong만 판별)	weakly=>1차원 벡터   unlabeled => X
#index 변수 하나더 strong labeled data를 나타낼 수 있는 입력 변수 추가
#target 중에서 어디서부터 어디까지 strong인지
# weak-labeled data   0-5     6
# unlabel_data        6-17    12
# strong-labeled data 18-23   6
...
def train(train_loader, model, optimizer, c_epoch, ema_model=None, mask_weak=None, mask_strong=None, adjust_lr=False):
    ...
    for i, ((batch_input, ema_batch_input), target) in enumerate(train_loader):
        ...
        '''
        type(batch_input): <class 'torch.Tensor'>, batch_input.shape: torch.Size([bs(batch size)=24, channel(depth)=1, time=628, frequency=128])
        type(target): <class 'torch.Tensor'>, target.shape: torch.Size([batch_size=24, time=157, nclass=10])
        target 사이즈 확인하기1.	사이즈에 대한 숫자들의 의미 파악하기
        '''
        ...
        #shifting_,target_s = shifting(batch_input,target)
        #plt_(batch_input,target,shifting_,target_s)

        #mix_up_, target_s = mix_up(batch_input,0.5,target)
        #plt_mix_up(batch_input,target,mix_up_,target_s)

        #ema_batch_input = G_noise(batch_input,snr=30)
        #plt_(batch_input,target,g_noise,target)

        #time_mask_ = time_mask(batch_input, labels=None, net_pooling=None, mask_ratios=(10, 50))
        #plt_(batch_input,target,time_mask_,target)

        #frequency = frequency_mask(batch_input, mask_ratios=(10, 50))
        #plt_(batch_input,target,frequency,target)

        #ema_batch_input= filt_aug(ema_batch_input.clone())
        # plt_(batch_input,target,ema_batch_input,target)

        # filter= mask_magnitude_plus(batch_input.clone())
        # plt_(batch_input,target,filter,target)
        ...
        batch_input = ema_batch_input.clone()


        # Outputs
        strong_pred_ema, weak_pred_ema = ema_model(ema_batch_input)     #strong_pred : 24*157*10
        strong_pred, weak_pred = model(batch_input)                     #weak_pred : 24*10
	##########################################################################################################

        strong_pred_ema = strong_pred_ema.detach()                  #더 이상 트레이닝 하지 말라는 테크닉
        weak_pred_ema = weak_pred_ema.detach()

        # ema_batch_input   => target_s (true)
        # batch_input       => target   (true)

        loss = None
        # torch.autograd.set_detect_anomaly(False)
        # Weak BCE Loss
        target_weak = target_cat.max(-2)[0]  # Take the max in the time axis,,, 타임축으로 합쳐서 없애버리는거
        mask_weak_2 = slice(24,30,None)
        mask_strong_2 = slice(42,48,None)
        if mask_weak is not None:
            a = torch.cat([weak_pred[mask_weak],weak_pred[mask_weak_2]],dim=0)
            b = torch.cat([target_weak[mask_weak],target_weak[mask_weak]],dim=0)
            c = torch.cat([weak_pred_ema[mask_weak],weak_pred_ema[mask_weak_2]],dim=0)
            weak_class_loss = class_criterion(a,b)
            ema_class_loss = class_criterion(c,b)
        ...
        if mask_strong is not None:
            a = torch.cat([strong_pred[mask_strong],strong_pred[mask_strong_2]],dim=0)
            # print(target.size(), strong_pred.size())
            b = torch.cat([target_cat[mask_strong],target_cat[mask_strong_2]],dim=0)
            c = torch.cat([strong_pred_ema[mask_strong],strong_pred_ema[mask_strong_2]],dim=0)
            strong_class_loss = class_criterion(a, b) #24*157*10
            meters.update('Strong loss', strong_class_loss.item())
            strong_ema_class_loss = class_criterion(c, b)
        ...
        if ema_model is not None:
            ...
            # batch_input     => student model => strong_pred     (original)    
            # ema_batch_input => teacher model => strong_pred_ema (time rolling)
